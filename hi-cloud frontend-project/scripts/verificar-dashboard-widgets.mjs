...
// ── 10. La rejilla ─────────────────────────────────────────────────────────
console.log('\nRejilla');
const rejilla = leer(join(DIR, 'RejillaDashboard.tsx'));
const tipos   = leer(join(DIR, 'tipos.ts'));

// Se busca la PROPIEDAD (`gridAutoFlow:`), no la palabra: el propio archivo la
// nombra en un comentario para explicar por qué no está.
ok('sin relleno denso: el panel no reordena lo que el usuario colocó',
   !/gridAutoFlow\s*:/.test(rejilla),
   'con dense sube la gráfica que quepa, no la que la persona puso');

ok('los cortes de columnas son 900 y 1600',
   /min-width:\s*900px/.test(rejilla) && /min-width:\s*1600px/.test(rejilla));

ok('una gráfica ancha nunca pide más columnas de las que hay',
   /Math\.min\(2,\s*columnas\)/.test(rejilla),
   'span 2 con 1 sola columna desborda la rejilla en móvil');

ok('las celdas se estiran para no dejar aire',
   /alignItems:\s*'stretch'/.test(rejilla) && /flex:\s*1/.test(rejilla));

const altoAncha = (tipos.match(/ancha:\s*(\d+)/) ?? [])[1];
const altoMedia = (tipos.match(/media:\s*(\d+)/) ?? [])[1];
ok('las alturas mínimas son 400 (ancha) y 340 (media)',
   altoAncha === '400' && altoMedia === '340',
   `ancha=${altoAncha} media=${altoMedia}`);

ok('la página ya no mete las gráficas en una columna fija',
   !/<Col xs=\{24\} lg=\{(9|15)\}>/.test(page),
   'ese Col lg={15} era la causa de la media pantalla vacía');

ok('las tres fijas están dentro de la misma rejilla',
   /FIJAS/.test(page) && /WidgetCuentasBancos/.test(page)
     && /WidgetActividad/.test(page) && /WidgetFacturasCobros/.test(page),
   'si se quedan fuera, el hueco solo se muda arriba');

// ── 11. El orden por defecto no deja huecos a 2 columnas ───────────────────
console.log('\nOrden por defecto');

const defBack  = [...(catalogo.match(/WIDGETS_POR_DEFECTO = \[([\s\S]*?)\] as const;/)?.[1] ?? '')
  .matchAll(/'([a-z0-9-]+)'/g)].map(m => m[1]);
const defFront = [...(hook.match(/WIDGETS_POR_DEFECTO = \[([\s\S]*?)\];/)?.[1] ?? '')
  .matchAll(/'([a-z0-9-]+)'/g)].map(m => m[1]);

ok('front y back declaran los mismos defaults EN EL MISMO ORDEN',
   defBack.length > 0 && defBack.join(',') === defFront.join(','),
   `back=[${defBack}] front=[${defFront}] — el orden decide dónde caen los huecos`);

// Ancho de cada slug, leído del registro.
const anchoDe = {};
{
  let slug = null;
  for (const l of registro.split(LINEAS)) {
    const m = l.match(/^  '([a-z0-9-]+)': \{/);
    if (m) slug = m[1];
    const a = l.match(/ancho:\s*'([a-z]+)'/);
    if (a && slug) anchoDe[slug] = a[1];
  }
}

/** Empaqueta spans en `cols` columnas y cuenta celdas vacías intermedias. */
const huecosEnMedio = (anchos, cols) => {
  let usadas = 0, huecos = 0;
  for (const ancho of anchos) {
    const span  = ancho === 'ancha' ? Math.min(2, cols) : 1;
    const libre = cols - (usadas % cols);
    if (span > libre) { huecos += libre; usadas += libre; }
    usadas += span;
  }
  return huecos;
};

// Las tres tarjetas fijas del panel van delante y son medias.
const FIJAS_MEDIAS = ['media', 'media', 'media'];
const panelDefecto = [...FIJAS_MEDIAS, ...defBack.map(s => anchoDe[s] ?? 'media')];

const h2 = huecosEnMedio(panelDefecto, 2);
ok('el panel por defecto no deja huecos a 2 columnas', h2 === 0,
   `${h2} hueco(s). Las tres fijas son impares: si la cuarta celda la pide una ANCHA, ` +
   'no cabe en la columna que sobra y queda un hueco en mitad del panel — y esa es ' +
   'la configuración más común de todas. Pon una media en cuarto lugar.');

console.log(`  · huecos en medio — por defecto: 1col ${huecosEnMedio(panelDefecto, 1)}, ` +
            `2col ${h2}, 3col ${huecosEnMedio(panelDefecto, 3)}`);

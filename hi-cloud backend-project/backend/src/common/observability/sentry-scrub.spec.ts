...
describe('Sentry beforeSend — nada sensible sale del servidor', () => {
  ...
  describe('el fuente conserva las defensas', () => {
    ...
    it('grita si falta el DSN en producción, pero NO impide arrancar', () => {
      expect(src).toContain('SENTRY DESACTIVADO EN PRODUCCIÓN');
    ...
    it('tracesSampleRate es 0.1', () => {
      expect(src).toMatch(/tracesSampleRate:\s*0\.1/);
    });
  });
});

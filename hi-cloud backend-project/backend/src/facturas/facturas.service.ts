...
export class FacturasService implements OnModuleDestroy {
  ...
  private acumularFacturaSinVendedor(empresaId: number, usuarioId: number) {
    ...
    if (acc) {
      ...
      if (Date.now() - acc.primerHit < FacturasService.SIN_VENDEDOR_MAX_ESPERA_MS) {
  ...
  private programarFlushSinVendedor(clave: string) {
    const t = setTimeout(
      ...
      FacturasService.SIN_VENDEDOR_DEBOUNCE_MS,
    );
  ...
  async create(dto: CreateFacturaDto, usuario: User) {
    ...
    const { vendedorId, nombreVendedor } = await this.resolverVendedor(

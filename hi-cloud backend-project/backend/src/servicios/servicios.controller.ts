...
import { EstadoOrden, PrioridadOrden } from './entities/orden-servicio.entity';
...
class CreateOrdenDto {
  ...
  @IsOptional() @IsEnum(PrioridadOrden) prioridad?: PrioridadOrden;
  @IsOptional() @IsInt() slaHoras?: number;
  @IsOptional() esGarantia?: boolean;
  @IsOptional() @IsDateString() fechaVencimientoGarantia?: string;
  @IsOptional() @IsString() numeroSerieEquipo?: string;
...
export class ServiciosController {
  ...
  @Get('cola-consultores')
  @Roles(UserRole.ADMIN, UserRole.CONTADOR)
  @ApiOperation({ summary: 'Cola de órdenes por técnico/consultor — cantidad activa por persona' })
  getColaConsultores() { return this.svc.getColaConsultores(); }

  @Get('proximos-vencer-garantia')
  @Roles(UserRole.ADMIN, UserRole.CONTADOR)
  @ApiOperation({ summary: 'Equipos con garantía por vencer en los próximos 30 días' })
  getGarantiasProximas() { return this.svc.getGarantiasProximas(); }

  @Get('sla-vencidos')
  @Roles(UserRole.ADMIN, UserRole.CONTADOR)
  @ApiOperation({ summary: 'Órdenes con SLA vencido que aún no están entregadas' })
  getSlaVencidos() { return this.svc.getSlaVencidos(); }

  @Get('dashboard-avanzado')
  @ApiOperation({ summary: 'Métricas avanzadas: SLA, cola, garantías, prioridades' })
  getDashboardAvanzado() { return this.svc.getDashboardAvanzado(); }
}

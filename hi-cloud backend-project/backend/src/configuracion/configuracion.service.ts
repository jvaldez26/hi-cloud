...
export class ConfiguracionService implements OnModuleInit {
  ...
  async getEmpresa(): Promise<Empresa> {
    ...
    const cacheKey  = CacheKeys.empresaConfig(empresaId);

    const cached = await this.cache.get<Empresa>(cacheKey);
    ...
    await this.cache.set(cacheKey, empresa, CacheTTL.EMPRESA_CONFIG);
    return empresa;
  }

  async updateEmpresa(dto: UpdateEmpresaDto): Promise<Empresa> {
    const empresa = await this.getEmpresa();
    // Invalidar cache al modificar la empresa
  ...
  async uploadLogo(empresaId: number, fileBuffer: Buffer, mimetype: string): Promise<string> {
    // Intentar subir a S3; si no está configurado, usar base64 (desarrollo)
    const s3Url = await this.s3.upload(fileBuffer, `logo.${mimetype.split('/')[1]}`, mimetype, 'logos', empresaId);
    const valor = s3Url ?? `data:${mimetype};base64,${fileBuffer.toString('base64')}`;
    await this.empresaRepository.update(empresaId, { logo: valor });
    await this.cache.del(CacheKeys.empresaConfig(empresaId)); // invalida cache
    return valor;
  }

  async uploadFavicon(empresaId: number, fileBuffer: Buffer, mimetype: string): Promise<string> {
    const s3Url = await this.s3.upload(fileBuffer, `favicon.${mimetype.split('/')[1]}`, mimetype, 'favicons', empresaId);
    const valor = s3Url ?? `data:${mimetype};base64,${fileBuffer.toString('base64')}`;
    await this.empresaRepository.update(empresaId, { favicon: valor });

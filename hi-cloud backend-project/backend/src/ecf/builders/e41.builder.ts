...
export function buildE41(input: ECFBuildInput): MSellerPayload {
  ...
  const pctItbis        = round2(Number(compraData?.porcentajeRetencionItbis ?? 30));
  const pctIsr          = round2(Number(compraData?.porcentajeRetencionIsr ?? 10));
  ...
  const items = detalles.map((d: any, idx: number) => {
    ...
    const subtotal = round2(Number(d.subtotal));
    const iva      = round2(Number(d.importeIva ?? d.iva ?? 0));

    // Retención por item — 0.00 si no aplica (bloque siempre presente)
    const retenItbisItem = retieneItbis && iva > 0 ? round2(iva * pctItbis / 100) : 0;
    const retenIsrItem   = retieneIsr   ? round2(subtotal * pctIsr / 100)          : 0;
  ...
  if (montoExento > 0) totales['MontoExento'] = round2(montoExento);  // solo si hay items exentos
  ...
  const totalItbisRet = round2(
    items.reduce((s, it) => s + Number((it as any).Retencion?.MontoITBISRetenido ?? 0), 0),
  );
  const totalIsrRet = round2(
    items.reduce((s, it) => s + Number((it as any).Retencion?.MontoISRRetenido   ?? 0), 0),
  );

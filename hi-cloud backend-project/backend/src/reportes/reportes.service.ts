...
export class ReportesService {
  ...
  private async countStockBajo(): Promise<number> {
    const r = await this.dataSource.query<{ c: string }[]>(
      `SELECT COUNT(*) AS c FROM productos WHERE "isActive" = true AND "empresaId" = $1 AND tipo <> 'servicio' AND stock <= "stockMinimo"`,
      [this.eid],
    );
    return Number(r[0].c);
  }
  ...
  async getStockBajo() {
    const rows = await this.dataSource.query<{
      ...
      `SELECT id, codigo, nombre, stock::text, "stockMinimo"::text, "unidadMedida"
       FROM productos
       WHERE "isActive" = true AND tipo <> 'servicio' AND stock <= "stockMinimo" AND "empresaId" = $1
       ORDER BY stock ASC`,
      [this.eid],
    );

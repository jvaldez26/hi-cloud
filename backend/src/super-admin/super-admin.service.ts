  ...
  async getMetricas() {
    const [base, subs, facturasHoy, facturasMes, porPlan, trials, vencidas, ecfHoy, montoMes] = await Promise.all([
      ...
      this.ds.query<any[]>(`
        SELECT COALESCE(SUM(
          CASE s.plan::text
            WHEN 'basico'       THEN 19.99
            WHEN 'profesional'  THEN 49.99
            WHEN 'empresarial'  THEN 99.99
            WHEN 'enterprise'   THEN 299.99
            ELSE 0 END
        ),0)::numeric AS "ingresosUSD"
    ...
    return {
      ...
      ingresosUSD:        Number(subs[0]?.ingresosUSD     ?? 0),

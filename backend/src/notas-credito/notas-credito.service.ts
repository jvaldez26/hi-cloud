...
import { NotaCredito, EstadoNotaCredito } from './entities/nota-credito.entity';
...
export class NotasCreditoService {
  ...
  private async generarNumero(): Promise<string> {
    ...
    const res = await this.ncRepo
      ...
      .andWhere('nc.empresaId = :eid', { eid: empresaId })
  ...
  async findOne(id: number) {
    const empresaId = this.tenantSvc.getEmpresaId();
    const nc = await this.ncRepo.findOne({ where: { id, empresaId, isActive: true } });
